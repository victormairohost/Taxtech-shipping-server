...
// Define interface for document methods
...
const shipmentSchema = new Schema<ShipmentDocument, ShipmentModel>(
  {
    trackingNumber: {
      type: String,
      unique: true,
...
shipmentSchema.methods.canDelete = function (): boolean {
  return this.status === ShipmentStatus.PENDING;
};

// Export model
const Shipment = mongoose.model<ShipmentDocument, ShipmentModel>("Shipment", shipmentSchema);
